...
Phase 3.0 was subsequently corrected from one combined paragraph to dynamic event/company highlights. The deterministic service now screens up to 40 articles, verifies ticker evidence in article text, groups duplicate reporting into at most 18 event candidates, and makes one structured JSON request for no more than 10 distinct highlights. The validator derives sources and article counts from referenced candidate indices, rejects unsupported tickers and duplicate items, and returns insufficient-data status when fewer than three event candidates exist. The same explicit buttons and six-hour cache boundary remain in place.

## Phase 3.1 daily brief evidence

Each generated highlight now has a separate, concise `importance_reason` and a locally reconstructed citation list. The model may return only candidate indices; titles, URLs, sources, publishers, publication times, tickers, fallback state, source labels, and article counts are never trusted from model output. The service filters invalid and duplicate indices, retains no more than four citations in the model's stated relevance order, removes duplicate URLs, and permits only absolute HTTP(S) URLs to become links. Highlights without enough valid evidence are discarded, while missing importance text remains a safe unavailable state rather than a fabricated industry conclusion.

The component keeps the legacy summary and metadata card structure, adds a “why it matters” section, and places evidence in a collapsed citation expander. Missing links, sources, publishers, or dates are shown as incomplete metadata; fallback state is displayed only when present in the actual candidate. The Dashboard source selector, existing news cards, explicit generation and regeneration controls, one-request generation, six-hour cache, date/language/fingerprint/nonce key, provider boundaries, and fallback behavior are unchanged.
