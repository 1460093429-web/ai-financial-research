...
COMPANIES = [
    ...
    Company("Sandisk", "NAND flash storage", "SNDK", []),
    Company(
        "SK Hynix",
        "HBM memory",
        "000660.KS",
...
def get_analyst_targets(ticker: Any) -> Dict[str, Any]:
    ...
    median_target = target_value("median", "target_median_price")
    low_target = target_value("low", "target_low_price")
    high_target = target_value("high", "target_high_price")
    ...
    distribution_records = build_aggregate_proxy_records(
        ticker_symbol,
        target_price,
        median_target,
        low_target,
    ...
    distribution_records.extend(get_analyst_records(ticker_symbol))
